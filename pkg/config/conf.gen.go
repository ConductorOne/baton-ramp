...
type Ramp struct {
	...
	Provisioning     bool   `mapstructure:"provisioning"`
	AuditLogEvents   bool   `mapstructure:"audit-log-events"`
}
...
func (c *Ramp) GetStringSlice(fieldName string) []string {
...
func (c *Ramp) GetString(fieldName string) string {
	v, ok := c.findFieldByTag(fieldName)
	if !ok {
		return ""
	}
...
func (c *Ramp) GetBool(fieldName string) bool {
	v, ok := c.findFieldByTag(fieldName)
	if !ok {
		return false
	}
...
func (c *Ramp) GetStringMap(fieldName string) map[string]any {
	v, ok := c.findFieldByTag(fieldName)
	if !ok {
