...
- **Required only if `BATON_AUDIT_LOG_EVENTS=true`**: `audit_logs:read`.
...
Set `BATON_AUDIT_LOG_EVENTS=true` to expose Ramp's audit-log API as an event feed so vendor and agreement changes can flow into C1 incrementally between full syncs. This setting is independent from vendor-agreement resource enablement and requires the `audit_logs:read` scope. When the flag is unset (the default), the audit-log API is not called.
