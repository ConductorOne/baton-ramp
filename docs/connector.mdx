...
Synced Ramp vendors are emitted as C1 vendor resources with Ramp vendor identity metadata. Vendor owner access is represented as the `owner` entitlement on each vendor.
...
<Steps>
  ...
  <Step>
    ...
    If you enable **Sync vendor management data**, also select:

    - `business:read` — validate credentials, identify the Ramp business, and read audit-log events
    - `vendor_agreements:read` — sync vendor agreements
...
- **Always required**: `users:read`, `users:write`, `vendors:read`, `vendors:write`.
- **Required only if `BATON_VENDOR_MANAGEMENT=true`**: `business:read`, `vendor_agreements:read`.
...
The connector ships with an opt-in vendor-management surface. When `BATON_VENDOR_MANAGEMENT=true` (or the **Sync vendor management data** toggle is enabled in the C1 UI), the connector additionally:

- Decorates each Ramp vendor with cross-system identity and pre-aggregated trailing-window spend (30-day, 365-day, year-to-date).
- Syncs Ramp vendor agreements as their own resources, including term, renewal status, total contract value, and contract owners.
- Exposes Ramp's audit-log API as an event feed so vendor and agreement changes flow into C1 incrementally between full syncs.

This requires the additional `business:read` and `vendor_agreements:read` OAuth scopes above. When the flag is unset (the default), no vendor-management data is synced, no extra scopes are requested, and the audit-log API is not called.
...
stringData:
  ...
  # Optional: enable Ramp's vendor-management surface (vendor agreements,
  # pre-aggregated spend, audit-log incremental sync). Default off.
  BATON_VENDOR_MANAGEMENT: "true"
```
...
stringData:
  ...
  # Optional: enable Ramp's vendor-management surface (vendor agreements,
  # pre-aggregated spend, audit-log incremental sync). Default off.
  # When set, the OAuth client must also have the `business:read` and
  # `vendor_agreements:read` scopes.
  BATON_VENDOR_MANAGEMENT: "true"
```
