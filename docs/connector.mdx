...
- **Always required**: `users:read`, `users:write`, `vendors:read`, `vendors:write`, `business:read`.
- **Required only if `BATON_VENDOR_MANAGEMENT=true`**: `vendor_agreements:read`.
...
This requires the additional `vendor_agreements:read` OAuth scope above. When the flag is unset (the default), no vendor-management data is synced, no extra scopes are requested, and the audit-log API is not called.
...
stringData:
  ...
  # Optional: enable Ramp's vendor-management surface (vendor agreements,
  # pre-aggregated spend, audit-log incremental sync). Default off.
  # When set, the OAuth client must also have the `vendor_agreements:read`
  # scope.
  BATON_VENDOR_MANAGEMENT: "true"
```
